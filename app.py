...
#     webhook_url = f'https://api.telegram.org/bot{telegram_api_key}'/deleteWebhook"
...
def start_telegram():
    ...
    delete_webhook_url = f"https://api.telegram.org/bot{telegram_api_key}/deleteWebhook"
    requests.post(delete_webhook_url, json={"url": domain_url, "drop_pending_updates": True})
    
    # Set the webhook URL for the Telegram bot
    set_webhook_url = f"https://api.telegram.org/bot{telegram_api_key}/setWebhook?url={domain_url}/telegram"
    webhook_response = requests.post(set_webhook_url, json={"url": domain_url, "drop_pending_updates": True})
    print('webhook:', webhook_response)
    if webhook_response.status_code == 200:
        # set status message
        status = "The telegram bot is running. Please check with the telegram bot. @dsai_est_gemini_bot"
    else:
